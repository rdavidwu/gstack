...
[ -z "$_AUTO" ] && _AUTO=$(~/.claude/skills/gstack/bin/gstack-config get auto_upgrade 2>/dev/null || ~/.codex/skills/gstack/bin/gstack-config get auto_upgrade 2>/dev/null || .claude/skills/gstack/bin/gstack-config get auto_upgrade 2>/dev/null || .codex/skills/gstack/bin/gstack-config get auto_upgrade 2>/dev/null || true)
...
```bash
~/.claude/skills/gstack/bin/gstack-config set auto_upgrade true || ~/.codex/skills/gstack/bin/gstack-config set auto_upgrade true || .claude/skills/gstack/bin/gstack-config set auto_upgrade true || .codex/skills/gstack/bin/gstack-config set auto_upgrade true
```
...
**If "Never ask again":**
```bash
~/.claude/skills/gstack/bin/gstack-config set update_check false || ~/.codex/skills/gstack/bin/gstack-config set update_check false || .claude/skills/gstack/bin/gstack-config set update_check false || .codex/skills/gstack/bin/gstack-config set update_check false
```
Tell user: "Update checks disabled. Run `~/.claude/skills/gstack/bin/gstack-config set update_check true || ~/.codex/skills/gstack/bin/gstack-config set update_check true || .claude/skills/gstack/bin/gstack-config set update_check true || .codex/skills/gstack/bin/gstack-config set update_check true` to re-enable."
...
elif [ -d "$HOME/.codex/skills/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
...
if [ -z "$LOCAL_GSTACK" ] && [ -n "$_ROOT" ] && [ -d "$_ROOT/.codex/skills/gstack" ]; then
...
```bash
~/.claude/skills/gstack/bin/gstack-update-check --force 2>/dev/null || ~/.codex/skills/gstack/bin/gstack-update-check --force 2>/dev/null || .claude/skills/gstack/bin/gstack-update-check --force 2>/dev/null || .codex/skills/gstack/bin/gstack-update-check --force 2>/dev/null || true
```
...
**If versions differ:** follow the Step 4.5 sync bash block above to update the local copy from the primary. Tell user: "Global v{PRIMARY_VER} is up to date. Updated local vendored copy from v{LOCAL_VER} → v{PRIMARY_VER}. Commit that vendored `gstack/` directory when you're ready."
